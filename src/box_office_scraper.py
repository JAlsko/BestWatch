...
for i in range(5,6): #loop through all letter indices for movies
...
with open("moviedata.csv", "w") as f:
	...
	for url in movie_links:
		...
		current_url = (url + "&adjust_yr=2018&p=.htm")  #do all movies in 2015 dollars (done automatically by site with correct URL)
		...
		director_list=[]
		for t in directors:
			director_list.append(t.encode("utf-8"))
		for i in range(0,2):
		...
		writer_list=[]
		for t in writers:
			writer_list.append(t.encode("utf-8"))
		for i in range(0,2):
		...
		composer_list=[]
		for t in composers:
			composer_list.append(t.encode("utf-8"))
		for i in range(0,2):
		...
		actor_list=[]
		for t in actors:
			actor_list.append(t.encode("utf-8"))
		for i in range(0,6):
			if i>=len(actor_list):
		...
		producer_list=[]
		for t in producers:
			producer_list.append(t.encode("utf-8"))
		for i in range(0,6):
		...
		for t in all_bs:
			if 'Domestic Lifetime' not in t.encode("utf-8"):#want to ignore the lifetime box office
				b_list.append(t.encode("utf-8"))
